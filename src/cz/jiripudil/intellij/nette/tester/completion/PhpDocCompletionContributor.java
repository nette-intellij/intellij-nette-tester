...
public class PhpDocCompletionContributor extends CompletionContributor {
    public PhpDocCompletionContributor() {
        this.extend(CompletionType.BASIC, PlatformPatterns.psiElement().withParent(PhpDocPsiElement.class),
            new DocTagsCompletionProvider()
        );
    }

    private static class DocTagsCompletionProvider extends CompletionProvider<CompletionParameters> {
        private static String[] fileTags = new String[]{
            "@phpVersion", "@dataProvider", "@multiple", "@exitCode", "@httpCode",
            "@outputMatch", "@outputMatchFile", "@phpIni", "@phpExtension"
        };
        private static String[] fileTagsNoArgument = new String[]{"@testCase", "@skip"};
        private static String[] methodTags = new String[]{"@dataProvider"};
        ...
        private static boolean isDocCommentRedByTester(PhpDocComment docComment) {
            PhpDocComment commentRedByTester = TesterUtil.findDocCommentRedByTester(docComment.getContainingFile());
            return commentRedByTester != null && docComment.equals(commentRedByTester);
        }
